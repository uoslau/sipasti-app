...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-2">
            ...
            <h5 class="mb-0">{{ $can_manage ? 'Edit Kegiatan' : 'Detail Kegiatan' }}</h5>
        </div>
        @if ($can_manage)
            <div class="btn-group">
        ...
        @else
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-label-secondary">Hanya lihat</span>
                @if ($kegiatan->is_ob)
                    <a class="btn btn-primary {{ $kegiatan->is_generated ? '' : 'disabled' }}"
                        href="{{ route('kegiatanob.download', $kegiatan->slug) }}">Unduh</a>
                @else
                    <a class="btn btn-primary {{ $kegiatan->is_generated ? '' : 'disabled' }}"
                        href="{{ route('kegiatan.download', $kegiatan->slug) }}">Unduh</a>
                @endif
            </div>
        @endif
    </div>
    <div class="card-body">
                    ...
                    <div class="col-md-4">
                        <div class="mb-6">
                            ...
                            <input type="text" class="form-control @error('beban_anggaran') is-invalid @enderror"
                                id="beban_anggaran" name="beban_anggaran" placeholder="XXXX.XXX.XXX.005.A.521213"
        ...
        @else
            {{-- tampilan read-only untuk kegiatan milik tim kerja lain --}}
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-6">
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-6">
                        ...
                        <input type="text" class="form-control" value="{{ $kegiatan->tanggal_mulai }}" disabled>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-6">
                        <label class="form-label">Tanggal Selesai</label>
                        <input type="text" class="form-control" value="{{ $kegiatan->tanggal_selesai }}" disabled>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-6">
                        <label class="form-label">Beban Anggaran</label>
                        <input type="text" class="form-control" value="{{ $kegiatan->beban_anggaran }}" disabled>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-6">
                        ...
                        <input type="text" class="form-control"
                            value="{{ $kegiatan->tim_kerja_id == 12 ? $kegiatan->fungsi?->fungsi : $kegiatan->timKerja?->nama_tim_kerja }}"
                            disabled>
                    </div>
                </div>
                <div class="col-md-4">
            ...
            @if ($kegiatan->is_ob)
                <span class="badge bg-label-warning">Kegiatan O-B</span>
            @endif
            <small class="d-block mt-3 text-muted">Anda hanya dapat melihat kegiatan ini karena bukan bagian dari tim kerja Anda.</small>
        @endif
    </div>
</div>
